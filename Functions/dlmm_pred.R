# Prediction of Counterfactual risks

dlmm_pred <- function(basis, model = NULL, coef = NULL, vcov = NULL,
                      xscale = c(), lag = list(), bylag = c(), 
                      xmarg = c(), ci_level = 0.95) {
  
  if (!any(class(basis) %in% "dlmm_basis"))
    stop("basis must be from dlmm_basis")
  ...
  for (i in 1:M) {
    n <- x_names[i]
    idx <- grep(paste0("e", i, "\\.[0-9]{1,2}"), names(coef))
  ...
  if (M > 1) {
    for (i in 1:M) {
      for (j in i:M) {
        ...
        idx <- grep(paste0("m", i, "\\.[0-9]{1,2}\\.m", j, "\\.[0-9]{1,2}"), names(coef))
